...
export async function GET(req: NextRequest) {
  try {
    ...
    let args: string[] = [];
    if (isAudio) {
      args = ['-x', '--audio-format', 'mp3', '--audio-quality', '0', '--js-runtimes', 'node', '--extractor-args', 'youtube:player_client=ios,tv', ...cookieArgs, '-o', '-', ...ffmpegArgs, url];
    } else {
      args = ['-f', `${formatId}+bestaudio/best`, '--merge-output-format', 'mp4', '--js-runtimes', 'node', '--extractor-args', 'youtube:player_client=ios,tv', ...cookieArgs, '-o', '-', ...ffmpegArgs, url];
    }
